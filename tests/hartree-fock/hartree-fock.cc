           ...
           std::cout << "f1 is " << f1 << "f1234 is " << f1234 << std::endl;
            ...
            for(auto f2=0; f2!=n2; ++f2) {
              ...
              for(auto f3=0; f3!=n3; ++f3) {
                ...
                std::cout << " bf3 is " << bf3 << " bf3_first is " << bf3_first << std::endl; 
                #pragma omp parallel for shared(n4,s1234_deg,buf_1234,D,bf4_first,G,f1234) 
                for(int f4=0; f4<n4; ++f4) {
                  const int bf4 = f4 + bf4_first;

                  std::cout <<  " f4 is " << f4 << " n4 is " << n4 << " bf4 is " << bf4 << " bf4_first is " << bf4_first << std::endl;
                  std::cout << "(in loop) f1234 is " << f1234 << std::endl; 
                  const double value = buf_1234[f1234];

                  const double value_scal_by_deg = value * s1234_deg;
